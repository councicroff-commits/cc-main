...
// 🔥 DYNAMIC PERMANENT FIX for Clothes (Cleaned single slash)
const getApiBaseUrl = () => {
...
const FALLBACK_CARD: CategoryCard = {
  title: 'The Wardrobe',
  subtitle: 'Apparel Collection',
  description: 'Explore our complete selection of premium tops, bottoms, and essential outerwear designed for everyday refinement.',
  image: '',
  path: '/clothes'
};

export const ClothesPage: React.FC = () => {
  ...
  const fetchHomeData = useCallback(async () => {
    ...
    try {
      ...
      if (err.name !== 'AbortError') {
        console.error('[ClothesPage] Failed to fetch dynamic banner. Using fallback.', err);
        setApiError(true);
      } else {
        console.warn('[ClothesPage] Request timed out. Using fallback.');
        setApiError(true);
      }
    } finally {
  ...
  const pageCard: CategoryCard = useMemo(() => {
    ...
    const matchedCard = homeData.carousel.cards.find((c: CategoryCard) => c.path === '/clothes');
  ...
  const clothesProducts = useMemo(() => {
    return (products || []).filter(p => {
      ...
      return pCat === 'clothes' || pCat === 'apparel';
    });
  }, [products]);
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 pt-24 pb-24 px-4 sm:px-8 font-sans selection:bg-black selection:text-white">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-zinc-400">
          ...
          <span className="text-black px-1" aria-current="page">Clothes</span>
        </nav>
        ...
        <header className="relative w-full h-80 sm:h-96 rounded-3xl overflow-hidden shadow-2xl bg-zinc-900 border border-zinc-800">
          {pageCard.image ? (
            <img 
              src={pageCard.image} 
              alt="Apparel Collection Presentation" 
        ...
        <section className="space-y-6" aria-label="Clothes Products">
          <div className="flex justify-between items-center border-b border-zinc-200 pb-4">
            <h2 className="text-xs uppercase font-extrabold tracking-[0.2em] text-zinc-400">
              Showing {clothesProducts.length} Items
            </h2>
          </div>
          ...
          {isProductsLoading || isApiLoading ? (
            <div className="py-24 text-center text-zinc-400 text-xs font-bold uppercase tracking-widest" aria-live="polite">
              Loading apparel collection...
            </div>
          ) : clothesProducts.length > 0 ? (
            <ProductGrid products={clothesProducts} />
          ) : (
