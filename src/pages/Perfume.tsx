...
// 🔥 DYNAMIC PERMANENT FIX for Perfume (Cleaned single slash)
const getApiBaseUrl = () => {
...
const FALLBACK_CARD: CategoryCard = {
  ...
  subtitle: 'Fragrance Vault',
  description: 'Indulge in artisanal Extraits de Parfum and Eau de Parfums curated for lasting projection and subtle elegance.',
  image: '',
  path: '/perfume'
};
...
export const PerfumePage: React.FC = () => {
  ...
  const fetchHomeData = useCallback(async () => {
    ...
    try {
      ...
      if (err.name !== 'AbortError') {
        console.error('[PerfumePage] Failed to fetch dynamic banner. Using fallback.', err);
        setApiError(true);
      } else {
        console.warn('[PerfumePage] Request timed out. Using fallback.');
        setApiError(true);
      }
    } finally {
  ...
  const pageCard: CategoryCard = useMemo(() => {
    ...
    const matchedCard = homeData.carousel.cards.find((c: CategoryCard) => c.path === '/perfume');
  ...
  const perfumeProducts = useMemo(() => {
    return (products || []).filter(p => {
      ...
      return pCat === 'perfume' || pCat === 'fragrance';
    });
  }, [products]);
    ...
    <div className="min-h-screen bg-zinc-50 text-zinc-900 pt-24 pb-24 px-4 sm:px-8 font-sans selection:bg-black selection:text-white">
      <div className="max-w-6xl mx-auto space-y-12">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-xs font-semibold uppercase tracking-widest text-zinc-400">
          ...
          <span className="text-black px-1" aria-current="page">Perfume</span>
        </nav>
        ...
        <header className="relative w-full h-80 sm:h-96 rounded-3xl overflow-hidden shadow-2xl bg-zinc-900 border border-zinc-800">
          {pageCard.image ? (
            <img 
              src={pageCard.image} 
              alt="Fragrance Collection Presentation" 
        ...
        <section className="space-y-6" aria-label="Perfume Products">
          <div className="flex justify-between items-center border-b border-zinc-200 pb-4">
            <h2 className="text-xs uppercase font-extrabold tracking-[0.2em] text-zinc-400">
              Showing {perfumeProducts.length} Items
            </h2>
          </div>
